...
if [ "$ASK_OLLAMA" = 1 ]; then
  # TUI 는 줄바꿈이 곧 전송이라 개행을 모두 제거해야 한다.
  QTEXT=$(sed '/^[[:space:]]*$/d' "$OUT/question.md" | paste -sd '|' - | sed 's/|/ ▮ /g')

  echo "▸ ollama 패널에 질문 전달 (우하단)" >&2
  UNWRAP_FN=unwrap_prose send_and_wait "$OLLAMA_SURFACE" "$OLLAMA_BEGIN" "$OLLAMA_DONE" \
    "아래 ▮ 로 줄이 구분된 글은 어떤 분석 결과다. 이 분석이 타당한지 판단하라. 동의하는 점과 동의하지 않는 점을 각각 짧게 쓰고, 근거가 부족해 판단할 수 없는 항목은 '판단보류'라고 명시하라. 추측으로 사실을 만들어내지 마라. 답은 반드시 $OLLAMA_BEGIN 줄과 $OLLAMA_DONE 줄 사이에만 쓰라. 글: $QTEXT" \
