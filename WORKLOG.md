...
**codex 가 정정한 내 오류 2건**
1. **내부발열 44.5 W/㎡ 를 "아키타입 기본값"이라고 했는데 틀렸다.**
   `activity_schedules.py:84` 사무소 기본값은 **조명 9 + 기기 12 = 21 W/㎡** 다.
   나머지는 `calc_outlet_power_density()` 의 콘센트 부하(최대 25 W/㎡)가
   `outletLoadType="sum"` 기본값 때문에 **더해진** 것이다.
   → **아키타입 12 W/㎡ 가 이미 일반 콘센트를 포함한 기준이라면 이중계산이다.**
   용호동 내부부하 조사의 첫 번째 확인 대상.
