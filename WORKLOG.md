...
용호동 난방 6.6~7.0 vs 냉방 19.9~20.7 kWh/㎡·년. 서울 사무소로서 비상식적이다.

**지워진 후보**: 면적 기준 불일치(수정, 비율 불변) / 지면 열손실 누락(Ground 승격
민감도 +0.4, 6.1% — 기각) / 내부발열 과다(용도별 정정했으나 조명 19.4 + 기기 25.1 =
44.5 로 여전히 큼) / 기상 파일 오선택(성남→서울 정정, 기후 차이 작음).

**다음 후보**: 외피 성능. 단열재가 광물면 R13(λ=0.062)·광물보드 R-10.4 두 종뿐이고
창이 15개(WWR 매우 낮음)인데 이 조합이 난방부하를 이렇게까지 낮출 수 있는지 검증 필요.
6번 벤치마크를 먼저 도입해 엔진 신뢰구간을 잡는 편이 순서상 나을 수 있다.
